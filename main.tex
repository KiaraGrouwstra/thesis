...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        % \State calculate the type $t_s$ of each symbol $s \in S$;
        \State from $s \in S$ calculate expansion rules $r_s^{(1, \dots, n)}$ unrolling our grammar symbols; % into (partially) applied symbol variants;
        \State generate any possible program $p$ given our expansion rules $\forall s : r_s^{(1, \dots, n)} \in R^n$ and maximum number of holes;
        % \State calculate the type $t_p$ of each task function $p \in E$;
        \State sample up to a maximum number of monomorphic types $t^{(m)} \in T^{(m)}$ within a given nesting limit;
        % \State split the task functions' parameter types of $t_p$ into functions vs other;
        ...
        % \State recombine our generated sample expressions for each type instances across functions and non-functions;
...
% which like other neural methods is not presently making use of type-level information,
...
\emph{0}, % "zero"  -- did I need this in like foldr?
...
\begin{figure*}
    \begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|} \hline
        ...
        \textbf{typed} & 0.22 & 0.002 & 0.55 & 0.24 & 0.12 & 0.48 & 0.003 & 0.77 & 0.55 & 0.34 \\ \hline
        \textbf{uniform random} & 0.14 & 0.000 & 0.43 & 0.11 & 0.11 & 0.33 & 0.000 & 0.84 & 0.25 & 0.31 \\ \hline
    \end{tabular}
\caption{
    ...
    as well as mean accuracy separated by programs of a given number of nodes.
}
\label{fig:finalacc}
\end{figure*}
...
% TODO: put some conclusion somewhere with implications or w/e. also maybe like introduce sub-sections?
