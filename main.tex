...
Like them we also use the \emph{hyperbolic tangent} activation function in the \emph{multi-layer perceptrons} part of the \emph{R3NN}.
...
% then use these hyperparameters 
take the \emph{hyperparameters} values described in section \ref{sec:hpar}
...
For data structures we use \emph{List}, \emph{Maybe}, \emph{Tuple2}, \emph{Set}, and \emph{HashMap}.
...
% HashMap
\emph{insertHashMap}, % "HashMap.insert"
% Enum
...
%     \item[maxHoles (6)] the maximum number of holes to allow in a generated expression
...
we instead limit ours to a maximum of $6$,
...
% TODO: ditch validation as I'm considering dropping hyperparameter optimization?
% maxDataset
...
%     The \emph{multi-layer perceptrons} in the \emph{recursive} and \emph{reverse-recursive}
%     passes of the \emph{R3NN} are similarly fixed to using 3 layers.%
%     ~\footnote{
%         This value has been fixed statically, for both LSTMs and MLPs,
%         rather than provided as a parameter by the user.
%         % due to current limitations in their provided \emph{HaskTorch} implementations.
%     } 
%     % \item[seed] 
...
i.e. considering the synthesis a success if any of these exhibits the desired behavior.

% evalFreq
% TODO: if hyperparam optimization, validation set?
We evaluate performance on our test set once after every $5$ epochs of training.
% learningRate
...
% TODO: ditch this cuz honestly it doesn't do anything
