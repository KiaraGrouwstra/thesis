...
Whereas the use of example input-output pairs is only one of several options in the field of program synthesis,
fitting a model into the discrete form of a given grammar to produce a program forces one to instantiate such a model from probabilistic data interpretations,
...
characterized by differentiable models optimizable by backpropagation,
...
Program synthesis may instead construct a traditional program, which may be both deterministic as well as potentially faster to execute than predictions using the original machine learning model.
...
In program synthesis, one may take an existing program, and synthesize variants intended to generalize the existing logic to match the new data.
...
% TODO: conciliate this with the later section on human-machine cooperation
