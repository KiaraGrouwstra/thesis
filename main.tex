...
% TODO: introduce bigger picture, why I'm doing sections 'Types of program synthesis' and 'Existing approaches to programming by example'

\citet{gulwani2017program} introduce a taxonomy characterizing synthesizers by three key dimensions:
\begin{itemize}
...
% TODO: handle Emile's comment of making this description reproducible, including equations... even if the full original paper to me barely was reproducible.
...
also referred to as \emph{non-terminal leaf nodes},
...
The R3NN utilizes two parallel sets of neural networks both of one neural net per grammar production rule $r$,
using only a single layer with hyperbolic tangent activation function,
one set for each of its two passes through an AST: $f_r$ and $g_r$.
The R3NN first makes a \emph{recursive pass} from the leaves of the program tree through neural networks $f_r$ at each branch,
mapping from $Q \cdot M$ to $M$ dimensions,
all the way to the root node of the partial program,
making for an embedding of the full program so far.
...
At a leaf $l$, $r$ is determined by its symbol $S(l)$.
% phi, omega, phi', c, R(n) production rule of non-leaf node n, 

It then performs a \emph{reverse recursive pass} from this root,
...
\citet{nsps} define an \emph{expansion} as a combination of a hole and a grammar production rule, together making up for a way we can expand our PPT.
Expansion scores of an expension $e$ they define as $z_e = \phi'(e.l) \cdot \omega(e.r)$, with $e.l$ and $e.r$ indication an expansion's hole (leaf node) and production rule, respectively.
...
% TODO: what are you going to do: Introduce the model, what problem are you solving, etc.
